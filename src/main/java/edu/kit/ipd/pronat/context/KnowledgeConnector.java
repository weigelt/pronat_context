...
import edu.kit.ipd.pronat.context.data.*;
...
import edu.kit.ipd.pronat.context.data.relations.EntityConceptRelation;
...
import edu.kit.ipd.pronat.context.util.LeastCommonSubsumer;
 ...
 * new concepts from the Information in the {@link ContextIndividual}s.
 *
...
public class KnowledgeConnector implements IContextAnalyzer {
	...
	private void connectEntity(Entity entity) {
		if (entity instanceof SubjectEntity) {
			...
			for (ISystemClass system : systems) {
				if (subject.getGender().equals(SubjectEntity.Gender.NEUTRAL) || subject.getGender().equals(SubjectEntity.Gender.UNKNOWN)) {
	...
	private void extractSystemConcept(SubjectEntity subject, ISystemClass system, Double similarity) {
		...
		if (context.hasConceptOfIndividual(system.getFullName())
			...
			if (systemConcept.getGender().equals(SubjectEntity.Gender.UNKNOWN)) {
