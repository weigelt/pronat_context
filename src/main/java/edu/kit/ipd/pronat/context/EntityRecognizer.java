...
import edu.kit.ipd.pronat.context.data.relations.ConjunctionRelation;
...
import edu.kit.ipd.pronat.context.util.GenderNumberUtils;
import edu.kit.ipd.pronat.context.util.GraphUtils;
import edu.kit.ipd.pronat.context.util.WordNetUtils;
...
public class EntityRecognizer implements IContextAnalyzer {
	...
	private Entity getObjectEntity(List<INode> phrase) {
		...
		ObjectEntity.DeterminerType det = ObjectEntity.DeterminerType.UNKNOWN;
		...
		for (INode node : phrase) {
			...
			if (isNoun(node)) {
				...
				det = ObjectEntity.DeterminerType.SPECIFIC;
	...
	/**
	 * Retruns the {@link ObjectEntity.DeterminerType} of the specified lemma
	 *
	 * @param lemma
	 */
	private ObjectEntity.DeterminerType determineDeterminer(String lemma) {
		ObjectEntity.DeterminerType det;
		if (isGeneralDeterminer(lemma)) {
			det = ObjectEntity.DeterminerType.GENERAL;
		} else {
			det = ObjectEntity.DeterminerType.SPECIFIC;
		}
		return det;
	...
	/**
	 * Analyzes the specified name of any hints for {@link SubjectEntity.Gender}
	 *
	 * @param name
	 * @return
	 */
	private SubjectEntity.Gender getGender(String name) {
		...
		return SubjectEntity.Gender.UNKNOWN;

	}

	private SubjectEntity.Gender convertGender(Object gender) {
		if (Objects.equals(gender.toString(), SubjectEntity.Gender.MALE.name())) {
			return SubjectEntity.Gender.MALE;
		} else if (Objects.equals(gender.toString(), SubjectEntity.Gender.FEMALE.name())) {
			return SubjectEntity.Gender.FEMALE;
		} else if (Objects.equals(gender.toString(), SubjectEntity.Gender.NEUTRAL.name())) {
			return SubjectEntity.Gender.NEUTRAL;
		} else {
			return SubjectEntity.Gender.UNKNOWN;
		}
	}
